...
ASSERT(t.deselfer(testTable, reason + std.str.touppercase(reason)) = 'LEFT.reason + std . str . touppercase( LEFT.reason)', FAIL);

//Drop and select tests
DropResult := DATASET([{155, 'p'}, {245, 'p'}, {987, 'p'}, {987, 'p'}, {123,'p'}], {INTEGER diff, STRING Ps});   
ASSERT(t.drop(testTable, 'reason') = dropResult, FAIL);
ASSERT(t.select(testTable, 'diff, Ps') = dropResult, FAIL);
...
ASSERT(t.FilterSet(testTable, reason,  ['aa', 'baa']) = FilterResult1, FAIL);
ASSERT(t.FilterSet(testTable, reason,  ['aa', 'ca'], FALSE) = FilterResult2, FAIL);

ASSERT(t.Filter(testTable, filterDS, reason, filterColumn) = FilterResult1, FAIL);
ASSERT(t.Filter(testTable, filterDS1Col, reason, filterColumn) = FilterResult1, FAIL);
...
ASSERT(t.names(t.rename(testTable, diff, newDiff)) = t.names(renameResult), FAIL);
...
ASSERT(t.duplicated(testTable, reason) = DuplicatedResult, FAIL);
...
ASSERT(t.distinct(testTable, reason) = DistinctResult2, FAIL);
...
ASSERT(t.append(testTable, STRING, x, LEFT.reason + LEFT.reason) = AppendResult1, FAIL);
ASSERT(t.append(testTable, STRING, x, reason + reason) = AppendResult1, FAIL);
ASSERT(t.append(testTable, STRING, x, REGEXREPLACE('a', LEFT.reason + LEFT.reason, 'z')) = AppendResult2, FAIL);
ASSERT(t.append(testTable, STRING, x, REGEXREPLACE('a', reason + reason, 'z')) = AppendResult2, FAIL);
...
ASSERT(t.mutate(testTable, diff, 1) = mutateResult1, FAIL);
ASSERT(t.mutate(testTable, diff, LEFT.diff + 1) = mutateResult2, FAIL);
ASSERT(t.mutate(testTable, reason, REGEXREPLACE('a', LEFT.reason, 'z')) = mutateResult3, FAIL);
ASSERT(t.mutate(testTable, reason, REGEXREPLACE('a', reason, 'z')) = mutateResult3, FAIL);
ASSERT(t.mutate(testTable, reason, REGEXREPLACE('a', reason, 'z')) = mutateResult3, FAIL);
ASSERT(t.mutate(testTable, SELF.reason, REGEXREPLACE('a', LEFT.reason, 'z')) = mutateResult3, FAIL);

//arrane tests
arrangeResult := DATASET([{123,'ca', 'p'}, {155,'aa', 'p'}, {245,'baa', 'p'}, {987,'ca', 'p'}, {987,'ca', 'p'}], {INTEGER diff, STRING reason; STRING Ps;});   
ASSERT(t.arrange(testTable, diff) = arrangeResult, FAIL);

// CountN tests
coutnResult := DATASET([{'aa', 1}, {'baa', 1}, {'ca', 3}], {STRING reason; INTEGER n;}); 
t.arrange(t.CountN(testTable, 'reason'), reason);
t.arrange(t.CountN(testTable, 'reason, ps'), reason);
coutnResult;
// ASSERT(t.arrange(t.CountN(testTable, 'reason'), reason) = coutnResult, FAIL);
  
t.head(testTable);
t.head(filterDS, 2);
t.to_csv(testTable, '~ROB::TEMP::ACSV');
